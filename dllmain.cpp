...
auto write_audio_sample(OUTPUT_INFO const* const& oip, IMFSinkWriter* const& sink_writer, int const& n, DWORD const& index)
{
	...
	auto max_sample_size{ get_pcm_block_alignment(oip->audio_ch, 16) * oip->audio_rate };
	...
	auto sample_duration{ (static_cast<int64_t>(read_audio_sample_size / oip->audio_ch)) * 10'000'000 / oip->audio_rate };

	auto audio_buffer{ wil::com_ptr<IMFMediaBuffer>{} };
	THROW_IF_FAILED(MFCreateMemoryBuffer(read_audio_sample_size, &audio_buffer));
	BYTE* media_data{};
	DWORD media_data_max_length{};
	THROW_IF_FAILED(audio_buffer->Lock(&media_data, &media_data_max_length, nullptr));
	THROW_IF_FAILED(memcpy_s(media_data, read_audio_sample_size, audio_data, read_audio_sample_size));
	THROW_IF_FAILED(audio_buffer->Unlock());
	THROW_IF_FAILED(audio_buffer->SetCurrentLength(read_audio_sample_size));
	...
	THROW_IF_FAILED(audio_sample->SetSampleTime(n / oip->audio_rate * sample_duration));
