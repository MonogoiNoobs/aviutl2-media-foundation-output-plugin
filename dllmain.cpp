...
auto set_color_space_media_types(IMFMediaType *const &media_type, int32_t const &height)
{
	...
	if (height <= 720)
	{
		GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Color space desires BT.601.");
	...
	{
		GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Color space desires BT.2020 (12bit).");
	...
	{
		GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Color space desires BT.709.");
...
{
	GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Preparing DirectX Video Acceleration...");
	auto directx_device{ wil::com_ptr<IUnknown>{} };
	if (will_dx12_use)
	{
		GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"DirectX 12 is available so going to be used.");
...
{
	auto const sink_writer{ make_sink_writer(oip->savefile, is_accelerated, get_suitable_output_video_format_guid(std::filesystem::path(oip->savefile).extension(), GetPrivateProfileIntW(L"mp4", L"videoFormat", 0, configuration_ini_path))) };
...
auto write_audio_sample(OUTPUT_INFO const *const &oip, IMFSinkWriter *const &sink_writer, int32_t const &n, DWORD const &index, IMFMediaType *const &input_media_type)
{
	...
	if (n == 0) GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, std::format(L"audio: duration={}, time={}", sample_duration, sample_time).c_str());
...
auto write_samples(OUTPUT_INFO const *const &oip, sink_writer_with_indices_t &&sink_writer_with_indices, IMFMediaTypes const &input_media_types, bool const &is_accelerated)
{
	...
	GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Sending video samples to the writer...");
	...
	GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Sending audio samples to the writer...");
	...
	GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Finalizing. It may take a while...");
	sink_writer->Finalize();

	GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Done.");
	return;

abort:
	GLOBAL::aviutl_logger->info(GLOBAL::aviutl_logger, L"Aborted.");
	return;
}
auto output_file(OUTPUT_INFO const *const &oip, uint32_t const &video_quality, uint32_t const audio_bit_rate, bool const &is_hevc_preferred, bool const &is_accelerated)
{
...
auto func_output(OUTPUT_INFO *oip)
{
	...
	auto const video_quality{ GetPrivateProfileIntW(L"mp4", L"videoQuality", 70, mfop::configuration_ini_path) };
	auto const audio_bit_rate{ GetPrivateProfileIntW(L"mp4", L"audioBitRate", 3, mfop::configuration_ini_path) };
	auto const is_hevc_preferred{ GetPrivateProfileIntW(L"mp4", L"videoFormat", 0, mfop::configuration_ini_path) == 1 };
	auto const is_accelerated{ GetPrivateProfileIntW(L"general", L"useHardware", BST_UNCHECKED, mfop::configuration_ini_path) == BST_CHECKED };

	output_file(oip, video_quality, audio_bit_rate, is_hevc_preferred, is_accelerated);
...
intptr_t CALLBACK config_dialog_proc(HWND dialog, uint32_t message, WPARAM w_param, LPARAM)
{
	...
	case WM_INITDIALOG:
		...
		ComboBox_SetCurSel(GetDlgItem(dialog, IDC_COMBO2), GetPrivateProfileIntW(L"mp4", L"videoFormat", 0, mfop::configuration_ini_path));

		GetPrivateProfileStringW(L"mp4", L"videoQuality", L"70", &quality_wchar, 3, mfop::configuration_ini_path);
		THROW_IF_WIN32_BOOL_FALSE(SetDlgItemTextW(dialog, IDC_EDIT1, &quality_wchar));
		ComboBox_SetCurSel(GetDlgItem(dialog, IDC_COMBO1), GetPrivateProfileIntW(L"mp4", L"audioBitRate", 3, mfop::configuration_ini_path));

		Button_SetCheck(GetDlgItem(dialog, IDC_CHECK1), GetPrivateProfileIntW(L"general", L"useHardware", BST_UNCHECKED, mfop::configuration_ini_path));
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			_ltow_s(static_cast<long>(ComboBox_GetCurSel(GetDlgItem(dialog, IDC_COMBO2))), video_format_wchar, std::size(video_format_wchar), 10);
			THROW_IF_WIN32_BOOL_FALSE(WritePrivateProfileStringW(L"mp4", L"videoFormat", video_format_wchar, mfop::configuration_ini_path));

			GetDlgItemTextW(dialog, IDC_EDIT1, &quality_wchar, 3);
			THROW_IF_WIN32_BOOL_FALSE(WritePrivateProfileStringW(L"mp4", L"videoQuality", &quality_wchar, mfop::configuration_ini_path));

			_ltow_s(static_cast<long>(ComboBox_GetCurSel(GetDlgItem(dialog, IDC_COMBO1))), audio_bit_rate_wchar, std::size(audio_bit_rate_wchar), 10);
			THROW_IF_WIN32_BOOL_FALSE(WritePrivateProfileStringW(L"mp4", L"audioBitRate", audio_bit_rate_wchar, mfop::configuration_ini_path));

			THROW_IF_WIN32_BOOL_FALSE(WritePrivateProfileStringW(L"general", L"useHardware", Button_GetCheck(GetDlgItem(dialog, IDC_CHECK1)) ? L"1" : L"0", mfop::configuration_ini_path));
